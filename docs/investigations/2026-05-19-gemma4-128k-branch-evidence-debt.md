...
| 10 | hipGraph `kv_len` kernel-signature work (~7 attention kernels) | DEFERRED — touches every `attention_flash_*_window` kernel signature + Rust dispatch helpers. HIPFIRE_GRAPH stays default-off; no shipped path uses it. The fix recipe lives in the load-bearing comment block at `gemma4.rs:1568-1611`. |
